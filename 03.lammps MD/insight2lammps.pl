...
#  		July 6, 2011, August 14, 2012, April 30, 2013.
...
sub initialize {
  ...
  if ($class eq "") {
    print("Error: $selection is not a valid force field selection\n"); exit; }
  if ($class eq "class1") {
...
sub read_car_header {
  ...
  if ($PBC[0]>0) {
    ...
    @Box = (
      ...
      v_zero(@h[5,4,3]));
    $Triclinic = (
      $h[3]/$l[2]>=$ZERO)||($h[4]/$l[2]>=$ZERO)||($h[5]/$l[1]>=$ZERO);
  }
  else {
    my @min = (0,0,0);
    my @max = (0,0,0);
...
sub write_lammps_atoms {
  ...
  for ($n=1; $n<=$NAtoms; ++$n) {
    ...
    my $i; for ($i=0; $i<3; ++$i) { $pos[$i] = $arg[$i+1]; } #{ $pos[$i] = $arg[$i+1]+$offset[$i+3*$id]; }
    ...
    printf($LAMMPS " %14.9f %14.9f %14.9f", @pos);	# x, y, z # v_pbc(@pos)
	printf($LAMMPS "%s\n", $comment ? "  # $arg[6]" : "");
    $last = $arg[5];
  }
  printf($LAMMPS "\n");
